...
//    TX.publish(txBuffer);
 ...
 while ((subscription = mqtt.readSubscription())) {
   if (subscription == &RX) {
     ...
     if(comingValue == "STA4ENABLE"){
       Serial.println("ENB1");
       TX.publish("ESP32 STA4 ENABLED");
     }
     else if(comingValue == "ALLSTAENABLE"){
       Serial.println("ENB1");
       TX.publish("ESP32 STA4 ENABLED");
     }
     else if(comingValue == "STA4DISABLE"){
      TX.publish("ESP32 STA4 DISABLED");
     }
...
String encryiption(String key, String data){//33-125
  ...
  for(int i = 0; i < dataLen; i++){
    ...
    //Serial.println("Karakter : " + String(ch) + " ASCII : " + String(int(ch)));
  }
  //Serial.println("Sifrelenen Veri : " + String(output));
  return output;
//  Serial.println("Veri Uzunlugu : " + String(len));
  //delay(1000);
}
...
String decryiption(String key, String data){
  ...
  for(int i = 0; i < dataLen; i++){
    char dataCH = data[i];
    char keyCH = key[i%keyLen];
    output[i] = char((int(dataCH) - int(keyCH)) - 33 + 92);
    //Serial.println("Karakter : " + String(ch) + " ASCII : " + String(int(ch)));
  }
  //Serial.println("Cozulen Veri : " + String(output));
  return output;
//  Serial.println("Veri Uzunlugu : " + String(len));
  //delay(10000);
}
